    ...
    if(!empty($Student_Array)) {
        foreach ($Student_Array as $StudentRow) {
            ...
            <tr>
                ...
                <td><?php foreach ($Sex_array as $sex) {
                        $sexShow = $sex['SexID'];
                        if ($sexShow == $sexID) {
                            echo $sex['SexEN'];
                        }
                ...
                <td>
                    <?php foreach ($Semester_array as $Semester) {
                        $SemesterShow = $Semester['SemesterID'];
                        if ($semesterId == $SemesterShow) {
                            echo $Semester['SemesterEN'];
                        }
                    } ?>
                </td> <!--semester-->
                <td>
                    <?php foreach ($Major_array as $Major) {
                        $MajorShow = $Major['MajorID'];
                        if ($majorId == $MajorShow) {
                            echo $Major['MajorEN'];
                        }
                    } ?>
                </td> <!--Major-->
                <td>
                    <?php foreach ($Shift_array as $shift) {
                        $shiftShow = $shift['ShiftID'];
                        if ($shiftId == $shiftShow) {
                            echo $shift['ShiftEN'];
                        }
                    } ?>
                </td> <!--shift-->
                <td>
                    <?php foreach ($Campus_array as $Campus) {
                        $CampusShow = $Campus['CampusID'];
                        if ($campusId == $CampusShow) {
                            echo $Campus['CampusEN'];
                        }
                    } ?>
                </td> <!--Campus-->
                <td>
                    <?php if (1 == $statusID) {
                        echo $StatusShow = '<span class="badge rounded-pill bg-label-success">Active</span>';
                    } elseif (null == $statusID) {
                        echo $StatusShow = '<span class="badge rounded-pill bg-label-warning">Pending</span>';
                    } elseif (2 == $statusID) {
                        echo $StatusShow = '<span class="badge rounded-pill bg-label-warning">Pending</span>';
                    } elseif (3 == $statusID) {
                        echo $StatusShow = '<span class="badge rounded-pill bg-label-danger">Disable</span>';
                    }
                    ?>
                </td>
                <td>
        ...
        <tr width="100%">
            <td colspan="12">
                <strong><h6 class='text-danger text-center mt-3'>NO DATA FOUND</h6></strong>
            </td>
        </tr><!--id-->
        <?php
    }
